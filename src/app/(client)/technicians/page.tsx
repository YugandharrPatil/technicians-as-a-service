		...
		<div className="container mx-auto p-4">
			...
			<div className="mb-6 grid gap-4 rounded-lg border p-4 md:grid-cols-4">
				<div>
					...
					<Select value={filters.jobType} onValueChange={(value) => setFilters((prev) => ({ ...prev, jobType: value as JobType | "all" }))}>
						<SelectTrigger>
							<SelectValue />
						</SelectTrigger>
						<SelectContent>
				...
				<div>
					...
					<Select value={filters.city} onValueChange={(value) => setFilters((prev) => ({ ...prev, city: value as string | "all" }))}>
						<SelectTrigger>
			...
			{isLoading ? (
				...
				<div className="text-center text-destructive">Error loading technicians. Please try again.</div>
			) : technicians.length === 0 ? (
				<div className="text-center text-muted-foreground">No technicians found matching your filters.</div>
			) : (
						...
						<Card key={tech.id} className="hover:shadow-lg transition-shadow">
							<CardHeader>
								<div className="flex items-start gap-4">
									{tech.photo_url ? (
										<img src={tech.photo_url} alt={tech.name} className="h-16 w-16 rounded-lg object-cover border" />
									) : (
								...
								<div className="mb-4 space-y-3">
									...
									{tech.cities.length > 0 && (
										<div className="flex items-center gap-1.5 text-xs text-muted-foreground">
											...
											{tech.cities.length > 1 && <span className="text-muted-foreground/70">+{tech.cities.length - 1} more</span>}
										</div>
									)}
