...
import { ArrowLeft, Calendar, CheckCircle2, Image as ImageIcon, MapPin, Star } from "lucide-react";
						...
						<CardHeader>
							<div className="flex items-start gap-4">
								{technician.photo_url ? (
									<img src={technician.photo_url} alt={technician.name} className="h-20 w-20 rounded-lg object-cover border" />
								) : (
							...
							{technician.embedding && (
								<>
									...
									<div className="rounded-lg bg-muted p-3 text-xs text-muted-foreground">
										...
										{technician.embedding.updatedAt && <p>Last synced: {formatDate(technician.embedding.updatedAt)}</p>}
									</div>
								</>
							)}
						</CardContent>
						...
						<CardContent className="space-y-4">
							...
							<div className="text-xs text-center text-muted-foreground">
								{technician.cities.length > 0 && (
									<p>
										Serves: {technician.cities.slice(0, 2).join(", ")}
