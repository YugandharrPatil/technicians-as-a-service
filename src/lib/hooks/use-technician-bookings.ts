...
export function useTechnicianBookings(userId: string | undefined) {
	return useQuery({
		...
		queryFn: async () => {
			...
			const { data: technician } = await supabase.from("taas_technicians").select("id").eq("user_id", userId).single();

			if (!technician) return [];

			// Query bookings for this technician, joining users for client info
			const { data: bookings, error } = await supabase.from("taas_bookings").select("*, taas_users(display_name, email)").eq("technician_id", technician.id).order("created_at", { ascending: false });
