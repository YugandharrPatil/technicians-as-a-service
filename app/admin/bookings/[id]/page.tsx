...
export default function AdminBookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (<AdminGate><AdminBookingDetailContent id={id} /></AdminGate>);
}
...
function AdminBookingDetailContent({ id }: { id: string }) {
  ...
  return (
    <div className="container mx-auto max-w-4xl p-4">
      <Link href="/admin/bookings" className="mb-4 inline-block text-sm text-muted-foreground hover:text-foreground">← Back to Bookings</Link>
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between"><CardTitle>Booking Management</CardTitle><Badge>{booking.status}</Badge></div>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <Link href={`/admin/clients/${booking.client_id}`} className="text-sm text-primary underline">View Client Profile</Link>
            </div>
            {technician && (
              <div>
                <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground"><Wrench className="h-4 w-4" /><span className="font-medium">Technician</span></div>
                <p className="text-lg font-semibold">{technician.name}</p>
                <Link href={`/admin/technicians/${technician.id}`} className="text-sm text-primary underline">View Technician Profile</Link>
              </div>
            )}
          </div>
          <div>
            <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground"><Wrench className="h-4 w-4" /><span className="font-medium">Service Type</span></div>
