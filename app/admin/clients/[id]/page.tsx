...
import type { User, Booking, Technician } from '@/lib/types/database';
...
export default function AdminClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (<AdminGate><AdminClientDetailContent id={id} /></AdminGate>);
}
...
function AdminClientDetailContent({ id }: { id: string }) {
  ...
  return (
    <div className="container mx-auto max-w-4xl p-4">
      <Link href="/admin/clients" className="mb-4 inline-block text-sm text-muted-foreground hover:text-foreground">← Back to Clients</Link>
      <Card className="mb-6">
        <CardHeader><CardTitle>Client Profile</CardTitle></CardHeader>
        <CardContent>
          <div className="space-y-2">
            <p><strong>Name:</strong> {client.display_name || 'No Name'}</p>
      ...
      <div>
        ...
        {bookings.length === 0 ? (
          <Card><CardContent className="py-8 text-center text-muted-foreground">No bookings found for this client.</CardContent></Card>
        ) : (
              ...
              <Card key={booking.id}>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div><CardTitle>{booking.service_type}</CardTitle><CardDescription>{booking.technician_name || 'Unknown Technician'}</CardDescription></div>
                    <Badge>{booking.status}</Badge>
                  </div>
                </CardHeader>
                <CardContent><Link href={`/admin/bookings/${booking.id}`}><button className="text-primary underline">View Booking</button></Link></CardContent>
              </Card>
            ))}
          </div>
        )}
