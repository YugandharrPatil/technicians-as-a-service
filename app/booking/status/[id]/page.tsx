...
export default function BookingStatusPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'taas_bookings', filter: `id=eq.${id}` },
        async (payload) => {
          ...
          if (updatedBooking.status === 'completed' && updatedBooking.completed_by_client && updatedBooking.completed_by_technician && technician?.user_id) {
            const { data: existingReview } = await supabase
              ...
              .eq('reviewee_id', technician.user_id)
              .single();
  ...
  return (
    <div className="container mx-auto max-w-2xl p-4">
      <Link href="/account/bookings" className="mb-4 inline-block text-sm text-muted-foreground hover:text-foreground">← Back to Bookings</Link>
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          {technician && (
            <div>
              <div className="mb-2 flex items-center gap-2 text-sm text-muted-foreground"><User className="h-4 w-4" /><span className="font-medium">Technician</span></div>
          ...
          {(booking.status === 'accepted' || booking.status === 'confirmed') && (
            <div className="pt-4"><Link href={`/chat/${booking.id}`}><Button className="w-full"><MessageSquare className="mr-2 h-4 w-4" />Open Chat</Button></Link></div>
          )}
      ...
      <Dialog open={showCompletionDialog} onOpenChange={setShowCompletionDialog}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={async () => { setShowCompletionDialog(false); await handleMarkCompleted(); }} disabled={updating}>
