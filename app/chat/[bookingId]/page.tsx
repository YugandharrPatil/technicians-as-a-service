...
import type { Booking, Technician, User as DbUser, ChatMessage } from '@/lib/types/database';
...
export default function ChatPage({ params }: { params: Promise<{ bookingId: string }> }) {
  ...
  async function loadBooking() {
    ...
    try {
      ...
      if (bk.client_id === user.id) {
        ...
        if (tech) { setTechnician(tech as Technician); setOtherPartyName(tech.name); }
      } else {
        setUserRole('technician');
        const { data: clientData } = await supabase.from('taas_users').select('*').eq('id', bk.client_id).single();
        if (clientData) { setOtherPartyName((clientData as DbUser).display_name || (clientData as DbUser).email); }
  ...
  async function handleSendOffer(e: React.FormEvent) {
    ...
    try {
      ...
      if (isNaN(price) || price <= 0) { toast.error('Invalid price'); setSending(false); return; }
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>{booking.service_type} — {booking.address}</CardDescription>
            </div>
                  ...
                  <div className={`max-w-[75%] rounded-lg p-3 ${isMe ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                    ...
                    {hasOffer && (
                      <div className="mt-2 rounded bg-yellow-100 p-2 text-yellow-800 text-sm">
                        ...
                        {canAcceptOffer && (
                          <Button size="sm" className="mt-2 w-full" onClick={() => handleAcceptOffer(msg)}>Accept Offer</Button>
                        )}
                      </div>
                    )}
          ...
          <form onSubmit={handleSendMessage} className="flex gap-2">
            ...
            <Dialog open={showOfferDialog} onOpenChange={setShowOfferDialog}>
              <DialogTrigger asChild>
                <Button type="button" variant="outline" size="icon"><DollarSign className="h-4 w-4" /></Button>
              </DialogTrigger>
              <DialogContent>
                <DialogHeader>
